...
const GestionPedidos = () => {
  ...
  const cargarPedidos = async () => {
    try {
      ...
      const pedidosMapped: Pedido[] = (pedidosRaw as PedidoBackend[]).map(
        (p) => ({
  ...
  const columns: ColumnDef<Pedido>[] = [
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        const estado = row.getValue("estado") as Pedido["estado"];
        const className =
          estado === "Rechazado" ? "text-red-600 font-semibold" : "";
        return <span className={className}>{`S/ ${value.toFixed(2)}`}</span>;
      },
    },
    {
      id: "acciones",
  ...
  const handleSearch = (value: string) => {
    const res = allData.filter((p) =>
      `${p.cliente.nomCli} ${p.cliente.apeCli}`
        .toLowerCase()
  ...
  return (
    <div className="space-y-6 pb-20">
      ...
      <div className="flex justify-between items-center">
        ...
        <Button
          variant="outline"
          className="flex items-center gap-2 bg-secundario text-black"
        >
          ...
          <div className="bg-white rounded-md shadow-lg w-full max-w-2xl p-4">
            <div className="flex justify-between items-center mb-4">
              <h2 className="text-lg font-semibold">
                Detalles del Pedido{" "}
                {pedidoSeleccionado && `#${pedidoSeleccionado.idPed}`}
              </h2>
                  ...
                  <tbody>
                    {detalles.map((d) => (
                      <tr key={d.idDetalle} className="border-b last:border-0">
                        <td className="py-2">
                          {d.producto?.nomProd || "Producto"}
                        </td>
                        <td className="py-2 text-right">{d.cantidad}</td>
                        <td className="py-2 text-right">
                          S/ {Number(d.precioUnit).toFixed(2)}
                        </td>
                    ...
                    {pedidoSeleccionado && (
                      <tr>
                        ...
                        <td className="py-2 text-right font-bold">
                          S/ {Number(pedidoSeleccionado.total).toFixed(2)}
                        </td>
                      </tr>
                    )}
                  </tfoot>
